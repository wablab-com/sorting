    ...
    public function sort(array $array, IComparer $comparer, string $order = self::ORDER_ASCENDING, $preserveKeys = false): array
    {
        ...
        do {
            ...
            for($seeker = 1; $seeker <= $maxValueIndex; $seeker++) {
                if( ($order == static::ORDER_ASCENDING && $comparer->compare($arrayValues[$seeker-1], $arrayValues[$seeker]) > 0) || ($order == static::ORDER_DESCENDING && $comparer->compare($arrayValues[$seeker-1], $arrayValues[$seeker]) < 0)) {
